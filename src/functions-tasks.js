...
function getPolynom(...coefficients) {
  ...
  if (coeffCount === 0) return null;

  return (x) => {
    let sum = 0;

    for (let i = 0; i < coeffCount; i += 1) {
      sum += coefficients[i] * x ** (coeffCount - 1 - i);
    }
...
function logger(func, logFunc) {
  return function callWithLogging(...args) {
    const { name } = func;
    const argList = args.map((arg) => JSON.stringify(arg)).join();
    logFunc(`${name}(${argList}) starts`);
    const result = func.apply(this, args);
    logFunc(`${name}(${argList}) ends`);
    return result;
  };
}
